...
2. **Pre-generated audio**, produced from those `turns` — **entirely free, offline, and with no API key of any kind**:
   ...
   ```
   That's it — no `.env` entry, no account, no key. `scripts/generate-audio.ts` synthesizes every turn locally with **Piper TTS**, a local neural text-to-speech engine, using one specific voice model whose licence was verified for commercial redistribution (see the audit below — this is *not* macOS's built-in `say`: that was tried first and explicitly rejected once we confirmed Apple's macOS Software License Agreement permits System Voices for "personal, non-commercial use" only). The script never imports `fetch`/an HTTP client — zero cost is enforced by there being no code path to a paid API at all, not just by using a free tier. `en_GB-vctk-medium` is a **multi-speaker** model (~109 speaker ids in one file, spanning many different British/Irish accents and both genders); the script reads the model's own config to find how many speakers it has and picks an evenly-spread subset, assigning each unique speaker in a track a distinct one (`lib/content/audioVoiceAssignment.ts` — deterministic, so re-runs never reshuffle voices). Pace comes from `lib/content/listeningPace.ts` (Piper's `--length_scale`, plus the silence gap between turns — Section 1 slower with the longest pauses, Section 4 continuous with minimal pausing). Piper has no natural-language style/emotion control the way a cloud TTS API does, so pace is the one lever this pipeline controls — `ListeningTrack.speakerPersonas` (accent/age/tone per speaker) is still authored and validated as content-quality documentation, but the current pipeline can't act on it directly; see the honest quality note below. The per-turn WAV clips are mixed down with `ffmpeg` (a hard requirement — Piper's WAV output can't be naively concatenated). MP3s save to `assets/audio/<trackId>.mp3`, and `lib/content/audioRegistry.ts` is rebuilt from whatever `.mp3` files actually exist on disk (so it never drops a previously-generated track, including the 74 Astra-sourced ones already committed). `TranscriptAudioPlayer` checks that registry first and only falls back to text-to-speech for tracks it doesn't cover. Re-running the script skips any track that already has a file — delete it first to force a re-generation.
...
To migrate another track: add a `turns` array to it, a `speakerPersonas` entry per speaker, and the same `audioSource` shape (kind `'local_tts'`, provider, license, sourceUrl, attribution — see the 4 tracks in `lib/content/listening.ts`/`listening2.ts` for the exact pattern), then run `npm run audio:generate`. Also add its id to the `PENDING_AUDIO_GENERATION` allowlist in `__tests__/contentIntegrity.test.ts` until you've actually run the script and committed its `.mp3` — the "production audio coverage" test in that file fails on purpose for any turns-based track that's neither generated nor on that list, so it can never silently regress to the on-device fallback.
...
**Honest quality note.** Piper is a real neural (VITS) TTS model, not a formant synthesizer — meaningfully more natural than a classic robotic TTS voice, though not equal to a modern large cloud model. It has no expressive/emotional steering the way a cloud TTS API's `instructions` parameter does — only which speaker id and how fast (`--length_scale`). So the 4 proof-of-concept tracks will have distinct speakers (real accent/gender variety from VCTK's 109 speakers), correct IELTS-style pacing progression, and zero spoken labels, but will sound more "synthesized" and less "human performance" than a paid cloud model would have produced. That trade-off is the accepted cost of a licence-safe, zero-recurring-spend pipeline. Listen to the actual generated files before deciding they're production-ready — I can't evaluate audio quality myself from this environment.
