...
**Generating all remaining production audio (one command).** After pulling this batch, run exactly the same command as above — it synthesizes every one of the 44 prepared tracks (plus regenerates Self-Storage with its now-fixed, contrasting voices) in one run, skipping anything already generated, and rewrites `audioRegistry.ts` from whatever's on disk when it's done:
   ```bash
   npm run audio:generate
   ```
