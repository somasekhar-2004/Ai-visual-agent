...
`supabase db push` applies **migrations only** (schema — tables, columns, RLS) — it does **not** run anything in `supabase/seed/`. If you've ever seen `mock_attempts_mock_test_id_fkey` (foreign key violation, code `23503`) on a real device, this is almost always why: the schema is up to date, but the live database was never (re)seeded with the current content, so a mock test id the bundled app references doesn't exist as a row yet.
