...
To migrate another track (e.g. from the legacy Astra set — audit its content/questions first, and author fresh `speakerPersonas` rather than trusting any pre-existing audio it came with): add a `turns` array to it, a `speakerPersonas` entry per speaker with an explicit gender word or pronoun (so `genderOf`/`assignSpeakerCategories` in `audioVoiceAssignment.ts` can actually read it), and the same `audioSource` shape (kind `'local_tts'`, provider, license, sourceUrl, attribution — see any of the 48 production tracks in `lib/content/listening.ts` through `listening6.ts` for the exact pattern), then run `npm run audio:generate`. Also add its id to the `PENDING_AUDIO_GENERATION` allowlist in `__tests__/contentIntegrity.test.ts` until you've actually run the script and committed its `.mp3` — the "production audio coverage" test in that file fails on purpose for any turns-based track that's neither generated nor on that list, so it can never silently regress to the on-device fallback. The "48-track production library" test group in the same file also enforces the 12/12/12/12 shape and that every multi-speaker track gets distinct voice *categories*, not just distinct IDs, so this can't silently regress either.
