...
Row Level Security and table-level `GRANT`s are two separate layers — RLS only ever gets evaluated *after* Postgres confirms the calling role has a `GRANT` on the table at all. `0006_grants.sql` grants `SELECT` on every public content table (`mock_tests`, `mock_sections`, `reading_passages`, `listening_tracks`, `writing_prompts`, `speaking_topics`, `questions`, `lessons`, `vocabulary_words`, `grammar_lessons`, `grammar_questions`, `achievements`, `band_conversion_tables`) to `authenticated` — but not to `anon`, which is a real gap: `scripts/verify-content-in-db.ts` deliberately connects with only `EXPO_PUBLIC_SUPABASE_ANON_KEY` and no login (the same public, client-safe value the app itself ships with), so without an `anon` grant every single content-table check in that script fails with `permission denied for table ...` — not because the rows are missing, but because the anon role was never allowed to even ask. `0007_anon_content_grants.sql` fixes this: `SELECT`-only, only on the same read-only content tables, for `anon` specifically. No RLS policy changes, RLS is never disabled anywhere, and no owner-scoped table (`profiles`, `user_goals`, `mock_attempts`, ...) is touched for either role.
