...
2. **Pre-generated audio**, produced from those `turns` — **entirely free, offline, and with no API key of any kind**:
   ```bash
   brew install ffmpeg   # one-time — used to add natural pauses between speaker turns and mix down to mp3
   npm run audio:generate
   ```
   That's it — no `.env` entry, no account, no key. `scripts/generate-audio.ts` synthesizes every turn locally with macOS's built-in `say` command (zero cost by construction: there is no code path in this script that can reach a paid API — it never imports `fetch`/an HTTP client at all) and only runs on macOS. It discovers the English voices actually installed on your Mac at run time via `say -v ?` (nothing hardcoded, since exact voice names vary by macOS version and which ones you've downloaded), assigns each unique speaker in a track a distinct one (`lib/content/audioVoiceAssignment.ts` — deterministic, so re-runs never reshuffle voices), and synthesizes at a **per-IELTS-section speech rate** (`lib/content/listeningPace.ts` — Section 1 slower/165wpm with the longest pauses, Section 4 continuous/minimal pausing). `say` has no natural-language style/emotion control the way a cloud TTS API does, so pace (rate + pause length) is the one lever this pipeline controls — `ListeningTrack.speakerPersonas` (accent/age/tone per speaker) is still authored and validated as content-quality documentation, but the current pipeline can't act on it directly; see the honest quality note below. The per-turn clips are mixed down with `ffmpeg` (a hard requirement here — `say`'s AIFF output can't be naively concatenated). MP3s save to `assets/audio/<trackId>.mp3`, and `lib/content/audioRegistry.ts` is rebuilt from whatever `.mp3` files actually exist on disk (so it never drops a previously-generated track, including the 74 Astra-sourced ones already committed). `TranscriptAudioPlayer` checks that registry first and only falls back to text-to-speech for tracks it doesn't cover. Re-running the script skips any track that already has a file — delete it first to force a re-generation.

To migrate another track: add a `turns` array to it, a `speakerPersonas` entry per speaker, and an `audioSource: { kind: 'local_tts', provider: 'macos-say' }` (see the 4 tracks in `lib/content/listening.ts`/`listening2.ts` for the pattern), then run `npm run audio:generate`. Also add its id to the `PENDING_AUDIO_GENERATION` allowlist in `__tests__/contentIntegrity.test.ts` until you've actually run the script and committed its `.mp3` — the "production audio coverage" test in that file fails on purpose for any turns-based track that's neither generated nor on that list, so it can never silently regress to the on-device fallback.

**Cost: $0, always.** `ListeningAudioSourceKind` (`types/models.ts`) only has two values — `'local_tts'` and `'human_corpus'` — there is no "paid cloud TTS" option in the type at all, so a paid provider can't sneak back in through content data either. Neither `scripts/generate-audio.ts` nor anything else in the Listening pipeline makes a network call or reads an API key. This was previously built against OpenAI's `gpt-4o-mini-tts` (paid, ~$0.015/minute) — that entire path has been removed, not just defaulted-off.
...
**Real human speech vs. locally-generated audio.** Every listening track's `audioSource.kind` is either `'local_tts'` (macOS `say`, as above — zero cost) or `'human_corpus'` (a reused real recording, with a required `license`/`sourceUrl`/`attribution` validated by `lib/content/audioLicense.ts`, and every accepted licence explicitly permits commercial redistribution). Researched sources: Mozilla Common Voice (CC0, but isolated single-sentence ASR-training clips — no natural dialogue), OpenSLR/VCTK/LibriSpeech (CC BY 4.0, same isolated-sentence problem, and LibriSpeech is itself sourced from LibriVox), TED talks (CC BY-NC-ND — rejected outright, no commercial/derivative use), and LibriVox (Public Domain, explicitly commercial-use-safe, and includes both single-narrator non-fiction readings *and* multi-reader "dramatic reading" recordings). LibriVox's academic/non-fiction catalog is a genuinely strong match for Section 4's register (dense single-speaker informational monologue) and is the recommended next step for real human audio — it wasn't used for this proof-of-concept mock because verifying a candidate clip's transcript word-for-word against the actual audio needs either a human to listen to it or an ASR pass; the concrete, still-zero-cost methodology (documented for whoever picks this up): transcribe a candidate clip with the app's own `transcribe-audio` Edge Function (already built, already free at the Gemini free tier or self-hosted Whisper), have a human confirm the draft against the audio, only then author questions from the verified transcript.

**Honest quality note.** macOS's built-in voices range from quite natural ("Enhanced"/"Premium" neural voices, if installed) to clearly synthetic (the older compact voices, which is whatever ships by default before you download better ones in System Settings → Accessibility → Spoken Content). Unlike a cloud TTS API, `say` has no expressive/emotional steering — only which voice and how fast. So the 4 proof-of-concept tracks will have distinct speakers, correct IELTS-style pacing progression, and zero spoken labels, but will sound more "text-to-speech" and less "human performance" than a modern cloud model would have produced. That trade-off is the direct, accepted cost of zero recurring spend. If a Mac has only default (non-Enhanced) voices installed, quality will be noticeably more robotic — installing the free Enhanced/Premium voices in System Settings before running the script is recommended.
